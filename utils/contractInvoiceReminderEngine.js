...
async function runOneWindow(window, limit) {
  ...
  for (const invoice of rows) {
    try {
      ...
      } else if (!waEnabled && !smsGate.ok) {
        // Neither channel was even attempted (tenant has neither feature).
        // Don't keep retrying — mark as sent to skip on subsequent runs.
        await db.query(
          `UPDATE contract_invoices SET ${window.flagColumn} = NOW() WHERE id = $1`,
          [invoice.invoice_id]
        );
