...
function buildVoiceSystemPromptOverride({ tenant, businessContext, customerData, isSignedIn, lang = "en" }) {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // VOICE-FIX-6 — Per-tenant prompt snapshot path
  //
  // If the tenant is in the voice-feature whitelist AND has a stored
  // voice_prompt_snapshot (generated by utils/voicePromptGenerator.js), use
  // the snapshot's prompt as the BASE and append a small dynamic block
  // (date/time, customer, language lock, currency speaking rules) that
  // varies per session. The snapshot is the tenant-specific piece;
  // the dynamic block is the session-specific piece.
  //
  // If either gate fails → fall through to the legacy code-rendered path
  // (V4 baseline, the rolled-back working version). Tenants without a
  // snapshot keep working exactly as today.
  //
  // The whitelist is intentionally hardcoded for the rollout. Once the
  // feature is stable on Birdie we can:
  //   (a) bulk-generate for legacy tenants (al-razi, aqababooking, ...)
  //   (b) move to a tenants.features JSONB column for proper feature flags
  //   (c) gate by plan tier
  // ────────────────────────────────────────────────────────────────────────

  const VOICE_PROMPT_FEATURE_SLUGS = ["birdie-golf"];
  const featureEnabled = VOICE_PROMPT_FEATURE_SLUGS.includes(tenant?.slug);
  const snapshot = tenant?.voice_prompt_snapshot;
  const snapshotPrompt = (snapshot && typeof snapshot === "object" && typeof snapshot.prompt === "string")
    ? snapshot.prompt.trim()
    : null;

  if (featureEnabled && snapshotPrompt && snapshotPrompt.length >= 200) {
    // Build the session-specific dynamic block. This is the only prompt text
    // generated per session — everything else is from the stored snapshot.
    const dynamicBlock = buildSessionDynamicBlock({
      tenantTz, tzOffsetStr, nowStr, todayStr, tomorrowStr,
      lang, sessionLang, otherLang,
      isSignedIn, customerData, paymentSettings,
    });

    return `${snapshotPrompt}\n\n${dynamicBlock}`;
  }

  // ── Legacy V4 fallback path (unchanged from pre-VOICE-FIX-5) ─────────────
...
// ────────────────────────────────────────────────────────────────────────────
// VOICE-FIX-6 — Session dynamic block
//
// Used only when a tenant has a stored voice_prompt_snapshot. Appends the
// per-session details that change every call: date/time, signed-in customer,
// upcoming bookings, language lock, currency speaking rules. The snapshot
// itself is tenant-specific and session-agnostic; everything dynamic lives
// here.
// ────────────────────────────────────────────────────────────────────────────
function buildSessionDynamicBlock({
  tenantTz, tzOffsetStr, nowStr, todayStr, tomorrowStr,
  lang, sessionLang, otherLang,
  isSignedIn, customerData, paymentSettings,
}) {
  // Customer block — same logic as the legacy path, but emitted as a session-
  // level "right now" appendix.
  const customerBlock = isSignedIn && customerData
    ? formatCustomerForVoice(customerData, paymentSettings, tenantTz)
    : `CUSTOMER: Not signed in. Provide general business information only. Do NOT reveal other customers' data. If they want to book, gently let them know they'll need to sign in first.
...
${formatTenantPaymentMethods(paymentSettings)}`;
  ...
  // Language lock — uses the same VOICE-FIX-4 wording. The button's choice is
  // authoritative; the agent never switches mid-session.
  const languageBlock = `LANGUAGE LOCK (session is locked to ${sessionLang}):
- Always respond in ${sessionLang} regardless of what language the customer speaks. Never mix languages within a single response.
- If the customer speaks ${otherLang} during a ${sessionLang} session, respond in ${sessionLang} and gently let them know once: ${lang === "ar"
      ? '"الجلسة مضبوطة على العربية حالياً — يمكنك التبديل للإنجليزية من زر اللغة في الأعلى."'
      : '"This session is set to English — you can switch to Arabic using the language toggle at the top."'}
- After mentioning the language toggle once per session, do NOT keep repeating it. The customer can switch when they want.
- Service names ("Karaoke", "Golf Simulator", etc.) may stay in English even within Arabic responses — customers use them that way locally.
- Numbers, times, currencies, and dates follow ${sessionLang}: ${lang === "ar"
      ? 'use Arabic-language time phrasing ("الساعة الخامسة"), Arabic numerals are fine.'
      : 'use English ("five o\'clock", "twelve fifty piasters").'}
- These instructions apply both to your spoken output AND to the text reply returned by the ask_booking_assistant tool.`;

  // Date / time — appended fresh every session so the agent never works with
  // a stale "today".
  const dateBlock = `CURRENT DATE & TIME: ${nowStr}
- "Today" means: ${todayStr}
- "Tomorrow" means: ${tomorrowStr}
- Business timezone: ${tenantTz} (UTC offset: ${tzOffsetStr})
- When you call ask_booking_assistant with a time-sensitive question, always include the explicit date in YYYY-MM-DD format, never relative phrases like "tomorrow".
- When you call ask_booking_assistant to CREATE a booking, ALWAYS include the customer's chosen payment method explicitly in the query string (one of: membership, package, cash, card, cliq) AND set the tool's is_confirming parameter to true. Example query: "Confirm Sim Bay 1 on 2026-05-04 at 17:00 for 2 hours, paying cash". For card or CliQ, expect a redirect message back asking the customer to use the public Book now button — speak that message back and don't retry.`;

  return [
    "═══════════════════════════════════════════════════════════════════════",
    "SESSION CONTEXT (dynamic — refreshed each call)",
    "═══════════════════════════════════════════════════════════════════════",
    "",
    customerBlock,
    "",
