...
const ENTITLEMENT_STATUSES = new Set(["active","scheduled","expired","consumed","cancelled","archived"]);
const ENTITLEMENT_SOURCES  = new Set(["manual_grant","purchase","migration","admin_adjustment","system"]);
const TRANSACTION_TYPES    = new Set(["grant","purchase","adjustment","renewal","expiry","redemption","consumption_reversal","manual_correction"]);
...
// ---------------------------------------------------------------------------
// Prepaid schema guard
// ---------------------------------------------------------------------------
let _prepaidTablesReadyCache = null;
async function ensurePrepaidTablesExist() {
  if (_prepaidTablesReadyCache === true) return true;
  ...
  const row = r.rows?.[0] || {};
  const ok = !!row.prepaid_products && !!row.customer_prepaid_entitlements
    && !!row.prepaid_transactions && !!row.prepaid_redemptions;
  _prepaidTablesReadyCache = ok;
  return ok;
}

async function requirePrepaidTables(req, res, next) {
  try {
    if (!(await ensurePrepaidTablesExist())) {
      return res.status(503).json({ error: "Prepaid accounting schema is not installed.", code: "PREPAID_SCHEMA_MISSING" });
    }
    return next();
  } catch (err) {
...
// Value normalizers
// ---------------------------------------------------------------------------
function asText(value, fallback = "")    { return value == null ? fallback : String(value).trim(); }
function asNullableText(value)           { const v = asText(value, ""); return v ? v : null; }
function asInt(value, fallback = 0)      { const n = Number(value); return Number.isFinite(n) ? Math.trunc(n) : fallback; }
function asMoney(value, fallback = 0)    { const n = Number(value); return Number.isFinite(n) ? Number(n.toFixed(2)) : fallback; }
function asBool(value, fallback = false) { return typeof value === "boolean" ? value : fallback; }
function asJsonObject(value)             { return value && typeof value === "object" && !Array.isArray(value) ? value : {}; }
...
function normalizeRedemptionPayload(input) {
  const payload = input && typeof input === "object" ? input : {};
  const redemptionMode = REDEMPTION_MODES.has(payload.redemptionMode) ? payload.redemptionMode : "manual";
  return {
    customerId:       asInt(payload.customerId, 0),
...
// ---------------------------------------------------------------------------
// Exports — only what sub-files actually call directly
// ---------------------------------------------------------------------------
module.exports = {
  // Middleware
  ...
  resolveIncludedQuantityFromProductRow,
  // Helpers
  actorUserId, fetchTenantCurrency,
};
