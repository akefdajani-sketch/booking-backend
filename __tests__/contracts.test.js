...
describe('utils/contracts — helpers', () => {
  ...
  describe('resolveContractPrefix', () => {
    test('uses stored prefix when valid', () => {
    ...
    test('falls back to UPPER(LEFT(slug,3)) when prefix is null', () => {
      expect(contracts.resolveContractPrefix({ contract_number_prefix: null, slug: 'aqababooking' })).toBe('AQB');
    });
    test('ignores invalid stored prefix (lowercase) and falls back', () => {
      expect(contracts.resolveContractPrefix({ contract_number_prefix: 'aqb', slug: 'aqababooking' })).toBe('AQB');
    });
  ...
  describe('applyTemplate', () => {
    ...
    test('computes due_dates for each trigger type', () => {
      ...
      expect(out.snapshot[0].due_date).toBe('2026-05-15'); // signing day (no signedAt → start)
      // Wait: signedAt is null, so signing uses `new Date()`. Adjust expectation.
      // The helper uses signedAt || new Date(), so without mocking Date, we can only
      // check that monthly_on_first rolled to the 1st correctly:
  ...
  test('POST / surfaces exclusion constraint as 409', async () => {
    const txClient = {
      query: jest.fn()
        ...
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: [{ next_seq: 1 }] })
        .mockRejectedValueOnce(Object.assign(new Error('overlap'), { code: '23P01' })),
      release: jest.fn(),
    };
    mockConnect.mockResolvedValueOnce(txClient);
