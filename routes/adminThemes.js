...
import { sanitizeThemeTokens } from "../theme/validateTokens.js";
import { requireAdmin } from "../middleware/requireAdmin.js"; // use your existing admin auth middleware
import db from "../db.js"; // use your existing db client wrapper
...
router.get("/", requireAdmin, async (req, res) => {
  const { rows } = await db.query(
    `SELECT key, name, version, is_published, updated_at
...
// Get one theme (includes tokens)
router.get("/:key", requireAdmin, async (req, res) => {
  const { key } = req.params;
  const { rows } = await db.query(
    `SELECT key, name, version, is_published, tokens_json
     FROM platform_themes
     WHERE key = $1`,
    [key]
  );
...
router.post("/", requireAdmin, async (req, res) => {
  ...
  const { rows } = await db.query(
    `INSERT INTO platform_themes (key, name, tokens_json, is_published)
     VALUES ($1, $2, $3::jsonb, FALSE)
     RETURNING key, name, version, is_published`,
    [key, name, JSON.stringify(safeTokens)]
  );
  res.status(201).json({ theme: rows[0] });
});

// Update theme tokens (draft-safe)
