  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      try {
        ...
        // link when transitioning into 'signed'. Same pattern as create.js.
        if (nextStatus === 'signed' && current.status !== 'signed' && newInvoiceIds.length > 0) {
          setImmediate(async () => {
            try {
              ...
              logger.error(
                { err: notifErr.message, contractId: id, tenantId },
